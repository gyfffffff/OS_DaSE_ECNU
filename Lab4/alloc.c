 ...
 */
  register struct hole *hp, *prev_ptr, *best_fit= NIL_HOLE, *best_fit_prev=NIL_HOLE;
  phys_clicks old_base;
  unsigned int min_left = 2<<30;
  unsigned int left = 0;
  do {
        prev_ptr = NIL_HOLE;
	      hp = hole_head;
        while (hp != NIL_HOLE && hp->h_base < swap_base) {  /*iterate*/
          /* look for best_fit */
          if (hp->h_len >= clicks){
              left = hp->h_len-clicks;
              if(left < min_left){
                min_left = left;
                best_fit = hp;
